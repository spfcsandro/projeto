...
import static com.util.ConstantesUtilWeb.IDENTIFICADOR_MENSAGEM_ALTERADO_COM_SUCESSO;
import static com.util.ConstantesUtilWeb.IDENTIFICADOR_MENSAGEM_CADASTRADO_COM_SUCESSO;
import static com.util.ConstantesUtilWeb.IDENTIFICADOR_MENSAGEM_EXCLUIDO_COM_SUCESSO;
...
import static com.util.ConstantesUtilWeb.REGISTRO_ALTERADO_COM_SUCESSO;
import static com.util.ConstantesUtilWeb.REGISTRO_CADASTRADO_COM_SUCESSO;
import static com.util.ConstantesUtilWeb.REGISTRO_EXCLUIDO_COM_SUCESSO;
...
public abstract class AbstractBean<E> implements GenericBean<E> {
	...
	public HashMap<String, String> getMensagens() {
		...
		mensagens.put(IDENTIFICADOR_MENSAGEM_CADASTRADO_COM_SUCESSO,
				REGISTRO_CADASTRADO_COM_SUCESSO);
		mensagens.put(IDENTIFICADOR_MENSAGEM_ALTERADO_COM_SUCESSO,
				REGISTRO_ALTERADO_COM_SUCESSO);
		mensagens.put(IDENTIFICADOR_MENSAGEM_EXCLUIDO_COM_SUCESSO,
		...
		new VerificadorLancamentoException()
				.tratarIhRelancarExcecaoSemLimparEntidade(new CommandBean() {
					...
					public void execute() {
						...
						lancarSucesso(getMensagens().get(
								IDENTIFICADOR_MENSAGEM_CADASTRADO_COM_SUCESSO));
					}

/*					private void adicionarEmpresaSeNecessario(E entidade) {
						if (entidade instanceof SetadorEmpresa) {
							((SetadorEmpresa) entidade)
									.setEmpresa(getUsuarioLogado().getPessoa()
											.getEmpresa());
						}
					}   */
				});
		...
		new VerificadorLancamentoException()
				.tratarIhRelancarExcecaoSemLimparEntidade(new CommandBean() {
					...
					public void execute() {
						...
						setCadastrarVisivel(false);
						lancarSucesso(getMensagens().get(
								IDENTIFICADOR_MENSAGEM_ALTERADO_COM_SUCESSO));
					}
				});
	}
		...
		new VerificadorLancamentoException()
				.tratarIhRelancarExcecaoSemLimparEntidade(new CommandBean() {
					...
					public void execute() {
						...
						limparEntidade();
						lancarSucesso(getMensagens().get(
								IDENTIFICADOR_MENSAGEM_EXCLUIDO_COM_SUCESSO));
					}
				});
	}
	...
	public UserSession getUsuarioLogado() {
		UsernamePasswordAuthenticationToken authentication = (UsernamePasswordAuthenticationToken) SecurityContextHolder
				.getContext().getAuthentication();
		ProjetoMockStant userStant = (ProjetoMockStant) authentication
				.getPrincipal();
	...
	public void redirecionarParaTela(String tela) {
		try {
			getContext().getExternalContext().redirect(
					getContext().getExternalContext().getRequestContextPath()
							+ tela);
		} catch (IOException e) {
	...
	protected void lancarSucessoRedirecionandoTelaComCallBack(String mensagem,
			String urlCallBack) {
		try {
